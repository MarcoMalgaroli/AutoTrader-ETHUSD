...
def grid_search_mlp(
    ...
    Results are sorted by **RoverMDD** (descending).
    ...
    for idx, vals in enumerate(combos, 1):
        ...
        row = {
            **params,
            ...
            "total_return_pct": (summary["final_equity"] - bt_cfg["initial_capital"]) / bt_cfg["initial_capital"],
            "trades": summary["trades"],
            "hit_rate": summary["hit_rate"],
            "max_drawdown": summary["max_drawdown"],
            "RoverMDD": abs(((summary["final_equity"] - bt_cfg["initial_capital"]) / bt_cfg["initial_capital"]) / summary["max_drawdown"] if summary["max_drawdown"] != 0 else 0),
        ...
        print(f"  => equity=${row['final_equity']:,.2f}  "
              ...
              f"dd={row['max_drawdown']:.2%}  roc={row['RoverMDD']:.2f}  ({elapsed:.0f}s)")

    results_df = pd.DataFrame(results).sort_values("RoverMDD", ascending=False).reset_index(drop=True)
...
if __name__ == "__main__":
    ...
    print("\n\x1b[32;1m>>> Results saved to grid_search_mlp_results.csv\x1b[0m")
