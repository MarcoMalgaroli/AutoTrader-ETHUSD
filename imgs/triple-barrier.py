def display_last_candles_from_csv(csv_path: str = None, num_candles: int = 30):
    """Display the last `num_candles` candles from a CSV file using mplfinance."""
    ...
    data_path = csv_path or os.path.join(base_dir, "datasets", "final", "ETHUSD_D1_.csv")
    ...
    volume_col = "tick_volume" if "tick_volume" in raw.columns else None

    keep_cols = ["time", "open", "high", "low", "close"]
    if volume_col:
        keep_cols.append(volume_col)

    candles = raw[keep_cols].copy().rename(
        columns={
            "time": "Date",
            "open": "Open",
            "high": "High",
    ...
    last_candles = candles.tail(num_candles)
    ...
    mpf.plot(
        ...
        style=style,
        volume=False,
        title=f"Last {len(last_candles)} Candles - {os.path.basename(data_path)}",
        ylabel="Price (USD)",
        datetime_format="%Y-%m-%d",
        figratio=(14, 7),
        savefig=dict(fname="candles.png", dpi=150, bbox_inches="tight"),
    )


def triple_barrier_visualization():    
    """
    ...
    for ax, (*_, fname) in zip(axes, SCENARIOS):
        ...
        bbox = bbox.expanded(1.0 + PADDING, 1.0 + PADDING)
...
if __name__ == "__main__":
    display_last_candles_from_csv()
    #triple_barrier_visualization()